...
export function CustomerReferralWidget() {
  ...
  return (
    <div className="rounded-2xl border border-[#e8e1de] bg-[#f9f2ef] p-5">
      ...
      {link ? (
        <div className="flex items-center gap-2 p-2.5 rounded-xl bg-white border border-[#e8e1de]">
        ...
        <div className="h-8 animate-pulse rounded-xl bg-white" />
      )}
      {stats.total > 0 && (
        <p className="mt-2 text-xs text-[#717171]">{stats.completed} friend{stats.completed !== 1 ? 's' : ''} joined · {stats.total - stats.completed} pending</p>
      )}
    </div>
  )
}
