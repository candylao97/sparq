...
export async function POST(req: NextRequest) {
  ...
  switch (event.type) {
    ...
    // ─── Chargeback / Dispute Updated Event ─────────────────────────
    case 'charge.dispute.updated': {
      ...
      // Map Stripe dispute status → internal DisputeStatus
      const stripeStatus = stripeDispute.status
      let internalStatus: 'UNDER_REVIEW' | 'RESOLVED_NO_REFUND' | 'RESOLVED_REFUND' | null = null
      if (['warning_needs_response', 'needs_response', 'under_review'].includes(stripeStatus)) {
        internalStatus = 'UNDER_REVIEW'
      } else if (stripeStatus === 'won') {
        internalStatus = 'RESOLVED_NO_REFUND'
      } else if (stripeStatus === 'lost') {
        internalStatus = 'RESOLVED_REFUND'
      }
      ...
      console.log(`[charge.dispute.updated] Booking ${booking.id} — dispute status: ${stripeStatus}`)
      break
    }
    ...
    case 'account.external_account.updated': {
      // Fires when a provider adds or updates their bank account on Stripe Connect
      const externalAccount = event.data.object as Stripe.BankAccount | Stripe.Card
      const stripeAccountId = externalAccount.account as string | undefined
      ...
      const isCreated = event.type === 'account.external_account.created'
      await prisma.notification.create({
        data: {
          userId: profile.userId,
          type: 'GENERAL',
          title: isCreated ? 'Bank account connected' : 'Bank account updated',
          message: isCreated
            ? "Your bank account has been connected. You're ready to receive payouts!"
            : 'Your payout bank account has been updated.',
          link: '/dashboard/provider',
        },
      }).catch(() => {})

      console.log(`[${event.type}] Notified provider ${profile.userId} — Stripe account ${stripeAccountId}`)
      break
    }
